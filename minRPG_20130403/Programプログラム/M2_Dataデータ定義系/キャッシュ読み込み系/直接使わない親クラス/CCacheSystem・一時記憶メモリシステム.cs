...
namespace PublicDomain
{
    ...
    public interface ICachedObject
    {
        ...
        ///		CCacheSystem.OnResourceChanged
        /// を呼び出す。
        /// 
        /// リソースにアクセスした際にはマーカーをつける意味で
        ///		CCacheSystem.OnAccess
        /// を呼び出す。
        /// 
        ...
        /// </summary>
        CCacheSystem<ICachedObject> CacheSystem
        {
            get;
            set;
    ...
    /// </summary>
    public abstract class CCachedObject・一時記憶メモリシステム : ICachedObject
    {
        ...
        {
            constructInfo・オブジェクト再構築データ = obj; //  as TextrueConstructAdaptor;
            isLoadFailed = false;
        }
        ...
        /// 再構築するときに必要となるパラメータ。
        /// </summary>
        public /*TextrueConstructAdaptor*/ object constructInfo・オブジェクト再構築データ = null;

        public void Reconstruct()
        {
            // ※さっきLostを確認したばっかり．現段階では，Lostに関係なく再生
            //if (IsLost)
            //{
                // 派生クラス側で再実装してくれい。
                YanesdkResult result = OnReconstruct(constructInfo・オブジェクト再構築データ); // ここでSound.Lodeにいけば再生！
        ...
        public void Destruct()
        {
            if (mix_in_this.Loaded)
            {
                ...
                object info = constructInfo・オブジェクト再構築データ;
                mix_in_this.Release();
                constructInfo・オブジェクト再構築データ = info;
            }
        }

        public virtual bool Reconstructable { get { return constructInfo・オブジェクト再構築データ != null; } }

        public bool IsLost
        {
            get
            {
                return !mix_in_this.Loaded && constructInfo・オブジェクト再構築データ != null && !isLoadFailed;
        ...
        /// </summary>
        public CCacheSystem<ICachedObject> CacheSystem
        {
            get { return cacheSystem; }
            set { cacheSystem = value; }
        }
        private CCacheSystem<ICachedObject> cacheSystem;
    ...
    /// <summary>
    ///	CCacheSystem は、それぞれのオブジェクトのリソースサイズを管理しており、
    ...
    /// CSoundPlayData・オーディオ再生用クラス/SoundLoader
    /// などで利用しているので、そちらも参考にすると良いだろう。
    /// </remarks>
    public class CCacheSystem<T> : IDisposable
        where
            T : ICachedObject
    {
        #region ctor & Dispose
		...
		/// </summary>
		public CCacheSystem<T> Parent
		{
			get { return parent; }
			set { Release・読み込み内容をリセット();  parent = value; }
		}
		private CCacheSystem<T> parent;
		*/
        ...
        public string DebugOutString()
        {
            ...
            foreach (CacheInfo info in list.List)
            {
                ...
                s.Append(
                    string.Format("p2_fileName・ファイル名 : {0} ,  AccessTime : {1} , ResourceSize : {2} , IsLost : {3} , Reconstructable : {4}\n",
                        filename,
