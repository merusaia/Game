...
using Sdl;
using Yanesdk.Draw;
using System.Windows.Forms; // このクラスはWindowsFormに依存している。Form非依存で使いたければ、CKeyBoardInputクラスで拡張して！
using Yanesdk.Ytl;
...

{
	...
	///	マウス入力定義クラス。
    ///	
    ///	※CMouseAndKeyboardKeyInput・キー入力定義クラスと機能がかぶっている部分もあり、
    ///	機能的に劣っている（ダブルクリックや長押しなどは取得できない）が、マウスの必要最小限のイベントを取っている。
    ///	このクラスでできることは、マウスの押されたボタンを判定することと、現在のマウスの座標を取ること。
    /// 
	/// 	/**
	/// 		ウィンドゥが生きていて、かつ、
	...
	*/
	public class CMouseInput・マウス入力定義 : IDisposable, IKeyInput・入力インタフェースに必要な機能定義クラス// merusaiaがIKeyInput継承を追加
    {

		public CMouseInput・マウス入力定義() {
			Reset();
		}
		...
		public CMouseInput・マウス入力定義(global::System.Windows.Forms.Control control)
		{
			// Windows専用の機能
		...
		/// </remarks>
		/// <param name="control"></param>
		public CMouseInput・マウス入力定義(global::System.Windows.Forms.Control control,bool onlyOnControl)
		{
			// Windows専用の機能
			if (Yanesdk.System.Platform.PlatformID != Yanesdk.System.PlatformID.Windows)
				throw new YanesdkException(YanesdkResult.NotImplemented);
		...
		/// 対するマウス位置を取得できる
		/// </summary>
		public CMouseInput・マウス入力定義(global::System.Windows.Forms.Form form)
		{
			Init(form , false);
		}
		/// <summary>
		...
		/// このクラスはマウスボタンが押されたと認識しない。
		/// </remarks>
		public CMouseInput・マウス入力定義(global::System.Windows.Forms.Form form , bool onlyOnForm)
		{
		...
		*/
		...
		/// <param name="_e"></param>
		/// <returns></returns>
		public bool IsPress(EMouseButton e)
		{
			bool b;
			switch (e)
			{
				case EMouseButton.Left  : b = left;   break;
				case EMouseButton.Middle: b = middle; break;
				case EMouseButton.Right : b = right;  break;
				case EMouseButton.XButton1: b = xbutton1; break;
				case EMouseButton.XButton2: b = xbutton2; break;
		...
		/// <param name="_e"></param>
		/// <returns></returns>
		private bool innerIsPress(EMouseButton e) {
		...
		/// <summary>
		/// 前々回のupdateの呼び出しのときにボタンが押されていて、（押されていなくて，の間違いでは？：メルサイア修正）
		/// 前回のupdate呼び出しのときにボタンが押されていれば
		/// このメソッドはtrueを返す。
		/// </summary>
		/// <param name="_e"></param>
		/// <returns></returns>
		public bool IsPush(EMouseButton e)
		{
			switch (e)
			{
				case EMouseButton.Left:	return !last_left && left;
				case EMouseButton.Middle: return !last_middle && middle;
				case EMouseButton.Right:  return !last_right && right;
				case EMouseButton.XButton1: return !last_xbutton1 && xbutton1;
				case EMouseButton.XButton2: return !last_xbutton2 && xbutton2;
			}
		...
		/// <param name="_e"></param>
		/// <returns></returns>
		public bool IsLeave(EMouseButton e)
		{
			switch (e)
			{
				case EMouseButton.Left: return last_left && !left;
		...
		public void Update()
		{
			...
			left	= innerIsPress(EMouseButton.Left);
			middle	= innerIsPress(EMouseButton.Middle);
			right	= innerIsPress(EMouseButton.Right);

			// Linux環境では無効だが。
			last_xbutton1 = xbutton1;
			last_xbutton2 = xbutton2;

			xbutton1 = innerIsPress(EMouseButton.XButton1);
			xbutton2 = innerIsPress(EMouseButton.XButton2);
        ...
        #region IKeyInput・入力インタフェースに必要な機能定義クラス メンバ //インタフェース継承のために、どれもmerusaiaがIKeyInputのコピペで作成。
        ...
        /// <param name="_e"></param>
        /// <returns></returns>
        public bool IsPush(int _buttonNo・EMouseButtonのint型)
        {
            switch (_buttonNo・EMouseButtonのint型)
            {
                case (int)EMouseButton.Left: return !last_left && left;
                case (int)EMouseButton.Middle: return !last_middle && middle;
                case (int)EMouseButton.Right: return !last_right && right;
                case (int)EMouseButton.XButton1: return !last_xbutton1 && xbutton1;
                case (int)EMouseButton.XButton2: return !last_xbutton2 && xbutton2;
            }
            throw null; // never reached
        }
        ...
        /// <param name="_e"></param>
        /// <returns></returns>
        public bool IsPress(int _buttonNo・EMouseButtonのint型)
        {
            bool b;
            switch (_buttonNo・EMouseButtonのint型)
            {
                case (int)EMouseButton.Left: b = left; break;
                case (int)EMouseButton.Middle: b = middle; break;
                case (int)EMouseButton.Right: b = right; break;
                case (int)EMouseButton.XButton1: b = xbutton1; break;
                case (int)EMouseButton.XButton2: b = xbutton2; break;
        ...
        /// <param name="_e"></param>
        /// <returns></returns>
        public bool IsPull(int _buttonNo・EMouseButtonのint型)
        {
            switch (_buttonNo・EMouseButtonのint型)
            {
                case (int)EMouseButton.Left: return last_left && !left;
        ...
        /// <summary>
        /// マウスボタンの前回・今回共に押されていない時に、trueを返します。
        /// </summary>
        /// <param name="_buttonNo・EMouseButtonのint型"></param>
        /// <returns></returns>
        public bool IsFree(int _buttonNo・EMouseButtonのint型)
        {
            switch (_buttonNo・EMouseButtonのint型)
            {
                case (int)EMouseButton.Left: return !last_left && !left;
        ...
        /// <summary>
        /// 定義されているマウスボタンの総数を取得します。
        /// </summary>
        /// <returns></returns>
        public int ButtonNum { get { return MyTools.getEnumIntMaxCount<EMouseButton>(); } }
