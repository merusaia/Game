...
namespace PublicDomain
{
	...
	/// ありません。
	/// </remarks>
	public class CJoyStick・ジョイスティック入力定義 : IKeyInput・入力インタフェースに必要な機能定義クラス {
		...
		/// 
		/// <code>
		///		int n = CJoyStick・ジョイスティック入力定義.countJoyStick();
		///		↑これで接続されているjoystickの本数を取得できる
		///
		///		CJoyStick・ジョイスティック入力定義[] joy = { new CJoyStick・ジョイスティック入力定義(0),new CJoyStick・ジョイスティック入力定義(1)};
		///		while (true) {
		...
		/// (エラーにはならない)
		/// </remarks>
		public CJoyStick・ジョイスティック入力定義(int nDeviceNo)
		{
		...
		/// 
		/// </summary>
		private IKeyInput・入力インタフェースに必要な機能定義クラス input;
	...
	internal class JoystickControl : IDisposable
	{
		...
		public JoystickControl()
		{
			if (init.Instance.Result < 0)
				...
				devices = new IKeyInput・入力インタフェースに必要な機能定義クラス[SDL.SDL_NumJoysticks()];
		}
		...
		/// deviceについては認識できﾅｰ
		/// </remarks>
		public IKeyInput・入力インタフェースに必要な機能定義クラス getDevice(int device_index) {
			...
			if (devices[device_index] == null){
				...
				if (j == IntPtr.Zero) // open失敗しちょる
					...
					devices[device_index] = new IJoyStickImp(j);
			} else {
				//	生成済みならそれを返せば?
			}
		...
		private IKeyInput・入力インタフェースに必要な機能定義クラス[] devices;
	...
	/// JoyStickの実装
	/// </summary>
	internal class IJoyStickImp : IKeyInput・入力インタフェースに必要な機能定義クラス
	{
		public void Dispose()
		{
		...
		// ~IJoyStickImp() { if (joystick == IntPtr.Zero) SDL.SDL_JoystickClose(joystick); }

		private IntPtr joystick;	//	SDLのjoystick識別子
		private bool[][] buttonState = new bool[2][];	//	前回の入力状態と今回の入力状態
		private int flip;	//	前回の入力状態と今回の入力状態をflipさせて使う。p_flip={0,1}
