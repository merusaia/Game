...

{
	...
	/// →SDLのimportをしたくない場合、
	/// 	このヘッダのなかにあるCKeyCodeというenumを用いても良い。
	/// </Para>
	/// <Para>
	/// 関数仕様についてはCJoyStickと同じなのでそちらを参照のこと。
	/// </Para>
	/// </remarks>
	public class old_CKeyBoardInput : IKeyInput・入力インタフェースに必要な機能定義クラス {
		...
		public void Update() {
			...
			else
			{
				...
				else
				{
					...
					for (int i = 0; i < keys.Length; i++)
					{
						p_buttonState[p_flip][i] = keys[i] != 0;
					}
				}
				 */
				// あかんこの方法ではうまいこといかへん。
		...
		private void Init()
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				buttonState[i] = new bool[(int)old_EKeyCode.LAST];
			}
		}
	...
	/*
		...
		private void OnKey(global::System.Windows.Forms.KeyEventArgs _e,bool press)
		{
			...
			switch (_e.KeyCode)
			{
				case Keys.A: p_buttonState[f][(int)KeyCode.a] = press; break;
				case Keys.B: p_buttonState[f][(int)KeyCode.b] = press; break;
				case Keys.C: p_buttonState[f][(int)KeyCode.c] = press; break;
			}
		}
	*/
		// あかん、この方法ではうまいこといかへん。
		...
		public void CheckKeyInWindows()
		{
			for (int i = 0; i < 10; ++i)
			{
				//--  0x30～0x39  メインキーボードのKEY0～9  
				buttonState[flip][(int)EKeyCode.KEY0 + i]
					= (GetAsyncKeyState(0x30 + i) & 0x8000) != 0;

				// -- NumKeyPadテンキーのKP0～9
				// VK_NUMPAD0==0x60
				buttonState[flip][(int)EKeyCode.RStick1_TenKeyPad0 + i]
					= (GetAsyncKeyState(0x60 + i) & 0x8000) != 0;
			}
			for (int i = 0; i < 26; ++i)
			{
				//--  0x41～0x5A  文字キー A から Z  
				buttonState[flip][(int)EKeyCode.a + i]
					= (GetAsyncKeyState(0x41 + i) & 0x8000) != 0;
			}
			// 全部のキーをサポートしても仕方がないのであとは主要キーのみにする

			// カーソル関連とNumPadの+,-,*,/
			buttonState[flip][(int)EKeyCode.UP] = (GetAsyncKeyState((int)VirtualKeyStates.VK_UP) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.DOWN] = (GetAsyncKeyState((int)VirtualKeyStates.VK_DOWN) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.RIGHT] = (GetAsyncKeyState((int)VirtualKeyStates.VK_RIGHT) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.LEFT] = (GetAsyncKeyState((int)VirtualKeyStates.VK_LEFT) & 0x8000) != 0;
			//buttonState[flip][(int)EKeyCode.RStick6_TenKeyPad_PLUS] = (GetAsyncKeyState((int)VirtualKeyStates.VK_ADD) & 0x8000) != 0;
			//buttonState[flip][(int)EKeyCode.RStick9_TenKeyPad_MINUS] = (GetAsyncKeyState((int)VirtualKeyStates.VK_SUBTRACT) & 0x8000) != 0;
			//buttonState[flip][(int)EKeyCode.RStick8_TenKeyPad_MULTIPLY] = (GetAsyncKeyState((int)VirtualKeyStates.VK_MULTIPLY) & 0x8000) != 0;
			//buttonState[flip][(int)EKeyCode.RStick7_TenKeyPad_DIVIDE] = (GetAsyncKeyState((int)VirtualKeyStates.VK_DIVIDE) & 0x8000) != 0;

			// あとはspace,enter,tab,alt,shift,ctrl,escぐらいでいいか。
			buttonState[flip][(int)EKeyCode.SPACE] = (GetAsyncKeyState((int)VirtualKeyStates.VK_SPACE) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.ENTER] = (GetAsyncKeyState((int)VirtualKeyStates.VK_RETURN) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.TAB] = (GetAsyncKeyState((int)VirtualKeyStates.VK_TAB) & 0x8000) != 0;

            // merusaia修正。WindowsVirtualKeyStates.VK_LWINはウィンドウズキー。ALTはVirtualKeyStates.VK_MENUのはず。
            buttonState[flip][(int)EKeyCode.ALT] = (GetAsyncKeyState((int)VirtualKeyStates.VK_MENU) & 0x8000) != 0;
			//buttonState[p_flip][(int)EKeyCode.LSUPER] = (GetAsyncKeyState((int)WindowsVirtualKeyStates.VK_LWIN) & 0x8000) != 0;
			//buttonState[p_flip][(int)EKeyCode.RSUPER] = (GetAsyncKeyState((int)WindowsVirtualKeyStates.VK_RWIN) & 0x8000) != 0;
			
            buttonState[flip][(int)EKeyCode.LSHIFT] = (GetAsyncKeyState((int)VirtualKeyStates.VK_LSHIFT) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.RSHIFT] = (GetAsyncKeyState((int)VirtualKeyStates.VK_RSHIFT) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.LCTRL] = (GetAsyncKeyState((int)VirtualKeyStates.VK_LCONTROL) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.RCTRL] = (GetAsyncKeyState((int)VirtualKeyStates.VK_RCONTROL) & 0x8000) != 0;
			buttonState[flip][(int)EKeyCode.ESCAPE] = (GetAsyncKeyState((int)VirtualKeyStates.VK_ESCAPE) & 0x8000) != 0;

            // merusaiaが追加。Backspace,Capslockもいれて
            buttonState[flip][(int)EKeyCode.BACKSPACE] = (GetAsyncKeyState((int)VirtualKeyStates.VK_BACK) & 0x8000) != 0;
            buttonState[flip][(int)EKeyCode.CAPSLOCK] = (GetAsyncKeyState((int)VirtualKeyStates.VK_OEM_ATTN) & 0x8000) != 0;

        }

        #region WindowsVirtualKeyStates -- Windowsの仮想キーコード表 わからないとき http://homepage3.nifty.com/ic/help/rmfunc/vkey.htm
		...
		/// <summary>
		/// 前回の入力状態と今回の入力状態をflipさせて使う。p_flip={0,1}
		/// </summary>
		private int flip;
	}

	/// <summary>
	/// CKeyboardInputで用いるキースキャンコード。
    /// 
    /// Shiftや左右別にLShiftとRShiftで認識するので注意。
    /// あと、Altは今のところ左右区別はしていない。
    /// あと、Fnはソフト的に単独では認識しないらしい。
	/// </summary>
	public enum old_EKeyCode : int {
		...
		SLASH		= 47,
		/// <summary>
		/// Key0～1～Key9は数字の０（をわのキ、テンキーはたぶん別ー）～１（！のキー）～９（）よのキー）。ＡＳＣＩＩコード用。
		/// </summary>
		KEY0			= 48,
		/// <summary>
		/// 数字の１（テンキーはたぶん別）
		...
		LCTRL		= 306,
		/// <summary>
        /// 右Altキー。現在はLALTと区別付けれてない。詳細はVirtualKeyStates.VK_MENUを検索して参照
		/// </summary>
		RALT		= 307,
		/// <summary>
		/// 左Altキー。ALTとキーコードは同じ。
