...
namespace PublicDomain
{
    ...
    /// ゲーム用のタスクの基底クラス
    /// </summary>
    public abstract class ITaskBase
    {
    ...
    /// <remarks>
    ///	タスクの管理を行ないます。
    ///	タスク( ITaskBase の派生クラス)を管理するために使います。
    ///
    ...
    /// </remarks>
    public class CTaskController・タスク管理者 : IDisposable
    {
        ...
        public class Task
        {
            ...
            /// タスク
            /// </summary>
            public CTaskBase・タスクベース TaskBase = null;
            ...
            public Task() { }
            public Task(CTaskBase・タスクベース task, int priority)
            {
                this.TaskBase = task;
        ...
        /// ITaskBase の Task メソッドの引数として渡されます。
        /// 
        ...
        {
            {
                ...
                while ( node != null )
                {
                    Task _nowTime = node.Value;
                    if ( !_nowTime.Dead )
                    {
                        if (!disableTaskList.ContainsKey(_nowTime.Priority))
                            if ( _nowTime.ITaskBase.Task(o) != 0 )
                                _nowTime.Dead = true; // こいつ殺す
                    }
                    node = node.Next;
                }
                 */
            ...
            foreach ( Task _nowTime in taskList )
            {
                if (!_nowTime.Dead && _nowTime.ITaskBase.Task(o) != 0 )
                    _nowTime.Dead = true; // こいつ殺す
            }
             */
            ...
            /*
            taskList.RemoveRemoveAll(delegate(Task _nowTime) {
                if (_nowTime.Dead)
                {
                    IDisposable d = _nowTime.ITaskBase as IDisposable;
                    if (d != null) d.Dispose();
                }
                return _nowTime.Dead;
            });
             */
        ...
        /// </Para>
        /// </remarks>
        public void AddTask(CTaskBase・タスクベース task, int priority)
        {
        ...
        */
        ...
        /// <param name="Priority"></param>
        /// <returns></returns>
        public CTaskBase・タスクベース GetTask(int priority)
        {
            foreach (Task t in taskList)
            {
    ...
    /// Task名を表現している列挙体
    /// </typeparam>
    public abstract class CTaskFactoryBase・タスク生成機ベース<TaskName>
    {
        ...
        /// </param>
        /// <returns></returns>
        public abstract CTaskBase・タスクベース CreateTask(TaskName name);
    }
    ...
    /// 
    /// </remarks>
    public class CSceneController・シーン管理者<TaskName> : CTaskBase・タスクベース, IDisposable
    {
        ...
        public void ReturnScene()
        {
            ...
            {
                TaskName name = PopTaskName();
                ITaskBase task = TaskFactory.CreateTask(name);
                taskList.Add(task);
            }
        }
        ...
        /// </remarks>
        /// <param name="c"></param>
        public CTaskFactoryBase・タスク生成機ベース<TaskName> TaskFactory;

        /*
            逐次実行の実現のためには、スタックさえあればそれで良い。
        */
        private List<TaskName> taskNameList = new List<TaskName>();	//	生成するtaskのpriorityを予約しておくキュー
        private List<ITaskBase> taskList = new List<ITaskBase>();		//	タスクのcallstack
        ...
        public void Reset()
        {
            foreach (ITaskBase t in taskList)
            {
        ...
        private void PopTask()
        {
            ...
            ITaskBase t = taskList[taskList.Count - 1];
            IDisposable d = t as IDisposable;
            if (d != null)
                d.Dispose();
